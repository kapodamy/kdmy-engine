...
Linkedlist linkedlist_init() {
    Linkedlist linkedlist = calloc(1, sizeof(Linkedlist_t));
    assert(linkedlist);
    return linkedlist;
}
...
bool linkedlist_has_item(Linkedlist linkedlist, void* obj_ptr) {
    _Linkedlist_node current = linkedlist->head;
    while (current) {
        if (current->item == obj_ptr) return true;
...
void* linkedlist_remove_item_at(Linkedlist linkedlist, int32_t index) {
    ...
    fprintf(stderr, "linkedlist (%p) is corrupted or in a invalid state", linkedlist);
    abort();
}
