...
static int script_character_use_alternate_sing_animations(lua_State* L) {
    READ_USERDATA(L, Character, character, CHARACTER);
...
static int script_character_update_reference_size(lua_State* L) {
    ...
    float width = luaL_optionalfloat(L, 2);
    float height = luaL_optionalfloat(L, 3);
...
static int script_character_set_offset(lua_State* L) {
    ...
    float offset_x = luaL_checkfloat(L, 2);
    float offset_y = luaL_checkfloat(L, 3);
...
static int script_character_play_sing(lua_State* L) {
    ...
    bool prefer_sustain = luaL_checkboolean(L, 3);
...
static int script_character_play_miss(lua_State* L) {
    ...
    const char* direction = luaL_optstring(L, 2, NULL);
    bool keep_in_hold = luaL_checkboolean(L, 3);
...
static int script_character_set_idle_speed(lua_State* L) {
    ...
    float speed = luaL_checkfloat(L, 2);
...
static int script_character_set_scale(lua_State* L) {
    ...
    float scale_factor = luaL_checkfloat(L, 2);
...
static int script_character_face_as_opponent(lua_State* L) {
    ...
    bool face_as_opponent = luaL_checkboolean(L, 2);
...
static int script_character_set_z_offset(lua_State* L) {
    ...
    float z_offset = luaL_checkfloat(L, 2);
...
static int script_character_get_modifier(lua_State* L) {
    ...
    return script_modifier_new(L, character, modifier);
}
...
static int script_character_has_direction(lua_State* L) {
    ...
    bool is_extra = luaL_checkboolean(L, 3);
...
static int script_character_get_current_action(lua_State* L) {
    ...
    const char* ret;
    switch (character_get_current_action(character)) {
...
int script_character_new(lua_State* L, Character character) {
    return NEW_USERDATA(L, CHARACTER, NULL, character, true);
}

static int script_character_gc(lua_State* L) {
    READ_USERDATA_UNCHECKED(L, Character, character, CHARACTER);
    _luascript_suppress_item(L, character, true);
    return 0;
}

static int script_character_tostring(lua_State* L) {
    luaL_checkudata(L, 1, CHARACTER);
    lua_pushstring(L, "[Character]");
    return 1;
}


inline void register_character(lua_State* L) {
    _luascript_register(
        L,
        CHARACTER,
        script_character_gc,
        script_character_tostring,
        CHARACTER_FUNCTIONS
    );
}
