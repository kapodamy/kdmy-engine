...
int32_t modding_launch_week(Modding modding, const char* week_name, char* difficult, bool alt_tracks, char* bf, char* gf, char* gameplay_manifest, int32_t track_idx) {
    print_stub("modding_launch_week", "modding=%p week_name=%s difficult=%s alt_tracks=(bool)%i bf=%s gf=%s gameplay_manifest=%s track_idx=%i", modding, week_name, difficult, alt_tracks, bf, gf, gameplay_manifest, track_idx);
    free(difficult);
    free(bf);
    free(gf);
