...
function weekselector_helptext_init(modelholder, layout, row, start, icon_name, string1, string2) {
    let font_size = layout_get_attached_value_as_float(layout, "help_text_font_size", 12.0);
    let font_name = layout_get_attached_value(layout, "help_text_font_name", LAYOUT_TYPE_STRING, null);

    let padding_x = layout_get_attached_value_as_float(layout, "help_text_padding_x", 0.0);
    let padding_y = layout_get_attached_value_as_float(layout, "help_text_padding_y", 0.0);
    ...
    let draw_x = padding_x * (start ? 2.0 : 1.0);
    ...
    if (modelholder) {
        ...
        sprite_resize_draw_size(helptext.icon, -1.0, font_size, size);
    ...
    if (start) {
        ...
        textsprite_set_max_draw_size(helptext.textsprite, helptext.max_width, -1.0);
        textsprite_set_draw_location(helptext.textsprite, draw_x, draw_y);
    }
