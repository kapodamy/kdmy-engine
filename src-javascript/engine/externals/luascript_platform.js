...
class LuascriptPlatform {
    ...
    #NotifyKeyboard(/**@type {KeyboardEvent}*/event, is_pressed) {
        ...
        const array = this.#luascript_instances.array;
        const size = this.#luascript_instances.size;
        for (let i = 0; i < size; i++) luascript_notify_input_keyboard(array[i], key, scancode, is_pressed, mods);
    }
    ...
    #CallbackMouseMove(event) {
        ...
        const array = this.#luascript_instances.array;
        const size = this.#luascript_instances.size;
        for (let i = 0; i < size; i++) luascript_notify_input_mouse_position(array[i], position_x, position_y);
    }

    #NotifyMouseButton(event, is_pressed) {
        ...
        const array = this.#luascript_instances.array;
        const size = this.#luascript_instances.size;
        for (let i = 0; i < size; i++) luascript_notify_input_mouse_button(array[i], button, is_pressed, mods);
    }

    #NotifyMouseHover(entering) {
        const array = this.#luascript_instances.array;
        const size = this.#luascript_instances.size;
        for (let i = 0; i < size; i++) luascript_notify_input_mouse_enter(array[i], entering);
    }

    #NotifyMouseWheel(evt) {
        this.#CallbackMouseMove(evt);
        ...
        const array = this.#luascript_instances.array;
        const size = this.#luascript_instances.size;
        for (let i = 0; i < size; i++) luascript_notify_input_mouse_scroll(array[i], deltaX, deltaY);
