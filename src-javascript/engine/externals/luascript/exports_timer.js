...
function script_timer_ms_gettime(L) {

    let result = timer_ms_gettime64();
...
function script_timer_register(L) {

    LUA.lua_pushcfunction(L, script_timer_ms_gettime);
    LUA.lua_setglobal(L, "timer_ms_gettime");

    const timer_sourcecode =
    ...
    let result = LUA.luaL_dostring(L, timer_sourcecode);
    console.assert(result == 0, "register_timer() failed");
}
