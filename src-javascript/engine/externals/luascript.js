...
ModuleLuaScript["preloadedImages"] = {};
ModuleLuaScript["preloadedAudios"] = {};
function abort(what) {
  {
...
function __js__character_enable_flip_correction(character, enable) {
  character_enable_flip_correction(kdmyEngine_obtain(character), enable);
}
function __js__character_flip_orientation(character, enable) {
  character_flip_orientation(kdmyEngine_obtain(character), enable);
}
...
function __js__layout_get_attached_value2(layout, name, result) {
  ...
  switch (type) {
    ...
    case LAYOUT_TYPE_INTEGER:
      dataView.setBigInt64(result, BigInt(value[0]), HEAP_ENDIANESS);
      break;
...
function __js__layout_get_placeholder_JS(layout, group_name, output) {
	...
	dataView.setFloat32(output, placeholder.parallax.x, HEAP_ENDIANESS);output += 4;
	dataView.setFloat32(output, placeholder.parallax.y, HEAP_ENDIANESS);output += 4;
	dataView.setFloat32(output, placeholder.parallax.z, HEAP_ENDIANESS);output += 4;

	dataView.setInt32(output, placeholder.static_camera ? 1 : 0, HEAP_ENDIANESS);

	return 1;
}
function __js__layout_disable_antialiasing(layout, disable) {
	layout_disable_antialiasing(kdmyEngine_obtain(layout), disable);
}
...
function __js__week_change_charecter_camera_name(roundcontext, opponent_or_player, new_name) {
...
function __asyncjs__fs_readfile(path, buffer_ptr, size_ptr) {
  return Asyncify.handleAsync(async () => {  
    try {
      ...
      if (ptr == 0) {
        console.error("__asyncjs__fs_readfile() out-of-memory, size was " + arraybuffer.byteLength);
        return 0;
      }
...
var asmLibraryArg = {
  ...
  "__js__week_change_charecter_camera_name": __js__week_change_charecter_camera_name,
