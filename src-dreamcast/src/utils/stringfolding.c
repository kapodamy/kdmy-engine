#include "stringbuilder.h"
#include "stringutils.h"

//
// Full Unicode case folding and unfolding (literally the whole unicode table)
// common caracters from a-z to A-Z are at the start to improve speed
// fold/unfold list taken from https://unicode.org/Public/UNIDATA/CaseFolding.txt
//


static const uint32_t STRING_UPPERCASES[] = {
    /*0x41,
    0x42,
    0x43,
    0x44,
    0x45,
    0x46,
    0x47,
    0x48,
    0x49,
    0x4A,
    0x4B,
    0x4C,
    0x4D,
    0x4E,
    0x4F,
    0x50,
    0x51,
    0x52,
    0x53,
    0x54,
    0x55,
    0x56,
    0x57,
    0x58,
    0x59,
    0x5A,*/
    0xB5,
    0xC0,
    0xC1,
    0xC2,
    0xC3,
    0xC4,
    0xC5,
    0xC6,
    0xC7,
    0xC8,
    0xC9,
    0xCA,
    0xCB,
    0xCC,
    0xCD,
    0xCE,
    0xCF,
    0xD0,
    0xD1,
    0xD2,
    0xD3,
    0xD4,
    0xD5,
    0xD6,
    0xD8,
    0xD9,
    0xDA,
    0xDB,
    0xDC,
    0xDD,
    0xDE,
    0x100,
    0x102,
    0x104,
    0x106,
    0x108,
    0x10A,
    0x10C,
    0x10E,
    0x110,
    0x112,
    0x114,
    0x116,
    0x118,
    0x11A,
    0x11C,
    0x11E,
    0x120,
    0x122,
    0x124,
    0x126,
    0x128,
    0x12A,
    0x12C,
    0x12E,
    0x132,
    0x134,
    0x136,
    0x139,
    0x13B,
    0x13D,
    0x13F,
    0x141,
    0x143,
    0x145,
    0x147,
    0x14A,
    0x14C,
    0x14E,
    0x150,
    0x152,
    0x154,
    0x156,
    0x158,
    0x15A,
    0x15C,
    0x15E,
    0x160,
    0x162,
    0x164,
    0x166,
    0x168,
    0x16A,
    0x16C,
    0x16E,
    0x170,
    0x172,
    0x174,
    0x176,
    0x178,
    0x179,
    0x17B,
    0x17D,
    0x17F,
    0x181,
    0x182,
    0x184,
    0x186,
    0x187,
    0x189,
    0x18A,
    0x18B,
    0x18E,
    0x18F,
    0x190,
    0x191,
    0x193,
    0x194,
    0x196,
    0x197,
    0x198,
    0x19C,
    0x19D,
    0x19F,
    0x1A0,
    0x1A2,
    0x1A4,
    0x1A6,
    0x1A7,
    0x1A9,
    0x1AC,
    0x1AE,
    0x1AF,
    0x1B1,
    0x1B2,
    0x1B3,
    0x1B5,
    0x1B7,
    0x1B8,
    0x1BC,
    0x1C4,
    0x1C5,
    0x1C7,
    0x1C8,
    0x1CA,
    0x1CB,
    0x1CD,
    0x1CF,
    0x1D1,
    0x1D3,
    0x1D5,
    0x1D7,
    0x1D9,
    0x1DB,
    0x1DE,
    0x1E0,
    0x1E2,
    0x1E4,
    0x1E6,
    0x1E8,
    0x1EA,
    0x1EC,
    0x1EE,
    0x1F1,
    0x1F2,
    0x1F4,
    0x1F6,
    0x1F7,
    0x1F8,
    0x1FA,
    0x1FC,
    0x1FE,
    0x200,
    0x202,
    0x204,
    0x206,
    0x208,
    0x20A,
    0x20C,
    0x20E,
    0x210,
    0x212,
    0x214,
    0x216,
    0x218,
    0x21A,
    0x21C,
    0x21E,
    0x220,
    0x222,
    0x224,
    0x226,
    0x228,
    0x22A,
    0x22C,
    0x22E,
    0x230,
    0x232,
    0x23A,
    0x23B,
    0x23D,
    0x23E,
    0x241,
    0x243,
    0x244,
    0x245,
    0x246,
    0x248,
    0x24A,
    0x24C,
    0x24E,
    0x345,
    0x370,
    0x372,
    0x376,
    0x37F,
    0x386,
    0x388,
    0x389,
    0x38A,
    0x38C,
    0x38E,
    0x38F,
    0x391,
    0x392,
    0x393,
    0x394,
    0x395,
    0x396,
    0x397,
    0x398,
    0x399,
    0x39A,
    0x39B,
    0x39C,
    0x39D,
    0x39E,
    0x39F,
    0x3A0,
    0x3A1,
    0x3A3,
    0x3A4,
    0x3A5,
    0x3A6,
    0x3A7,
    0x3A8,
    0x3A9,
    0x3AA,
    0x3AB,
    0x3C2,
    0x3CF,
    0x3D0,
    0x3D1,
    0x3D5,
    0x3D6,
    0x3D8,
    0x3DA,
    0x3DC,
    0x3DE,
    0x3E0,
    0x3E2,
    0x3E4,
    0x3E6,
    0x3E8,
    0x3EA,
    0x3EC,
    0x3EE,
    0x3F0,
    0x3F1,
    0x3F4,
    0x3F5,
    0x3F7,
    0x3F9,
    0x3FA,
    0x3FD,
    0x3FE,
    0x3FF,
    0x400,
    0x401,
    0x402,
    0x403,
    0x404,
    0x405,
    0x406,
    0x407,
    0x408,
    0x409,
    0x40A,
    0x40B,
    0x40C,
    0x40D,
    0x40E,
    0x40F,
    0x410,
    0x411,
    0x412,
    0x413,
    0x414,
    0x415,
    0x416,
    0x417,
    0x418,
    0x419,
    0x41A,
    0x41B,
    0x41C,
    0x41D,
    0x41E,
    0x41F,
    0x420,
    0x421,
    0x422,
    0x423,
    0x424,
    0x425,
    0x426,
    0x427,
    0x428,
    0x429,
    0x42A,
    0x42B,
    0x42C,
    0x42D,
    0x42E,
    0x42F,
    0x460,
    0x462,
    0x464,
    0x466,
    0x468,
    0x46A,
    0x46C,
    0x46E,
    0x470,
    0x472,
    0x474,
    0x476,
    0x478,
    0x47A,
    0x47C,
    0x47E,
    0x480,
    0x48A,
    0x48C,
    0x48E,
    0x490,
    0x492,
    0x494,
    0x496,
    0x498,
    0x49A,
    0x49C,
    0x49E,
    0x4A0,
    0x4A2,
    0x4A4,
    0x4A6,
    0x4A8,
    0x4AA,
    0x4AC,
    0x4AE,
    0x4B0,
    0x4B2,
    0x4B4,
    0x4B6,
    0x4B8,
    0x4BA,
    0x4BC,
    0x4BE,
    0x4C0,
    0x4C1,
    0x4C3,
    0x4C5,
    0x4C7,
    0x4C9,
    0x4CB,
    0x4CD,
    0x4D0,
    0x4D2,
    0x4D4,
    0x4D6,
    0x4D8,
    0x4DA,
    0x4DC,
    0x4DE,
    0x4E0,
    0x4E2,
    0x4E4,
    0x4E6,
    0x4E8,
    0x4EA,
    0x4EC,
    0x4EE,
    0x4F0,
    0x4F2,
    0x4F4,
    0x4F6,
    0x4F8,
    0x4FA,
    0x4FC,
    0x4FE,
    0x500,
    0x502,
    0x504,
    0x506,
    0x508,
    0x50A,
    0x50C,
    0x50E,
    0x510,
    0x512,
    0x514,
    0x516,
    0x518,
    0x51A,
    0x51C,
    0x51E,
    0x520,
    0x522,
    0x524,
    0x526,
    0x528,
    0x52A,
    0x52C,
    0x52E,
    0x531,
    0x532,
    0x533,
    0x534,
    0x535,
    0x536,
    0x537,
    0x538,
    0x539,
    0x53A,
    0x53B,
    0x53C,
    0x53D,
    0x53E,
    0x53F,
    0x540,
    0x541,
    0x542,
    0x543,
    0x544,
    0x545,
    0x546,
    0x547,
    0x548,
    0x549,
    0x54A,
    0x54B,
    0x54C,
    0x54D,
    0x54E,
    0x54F,
    0x550,
    0x551,
    0x552,
    0x553,
    0x554,
    0x555,
    0x556,
    0x10A0,
    0x10A1,
    0x10A2,
    0x10A3,
    0x10A4,
    0x10A5,
    0x10A6,
    0x10A7,
    0x10A8,
    0x10A9,
    0x10AA,
    0x10AB,
    0x10AC,
    0x10AD,
    0x10AE,
    0x10AF,
    0x10B0,
    0x10B1,
    0x10B2,
    0x10B3,
    0x10B4,
    0x10B5,
    0x10B6,
    0x10B7,
    0x10B8,
    0x10B9,
    0x10BA,
    0x10BB,
    0x10BC,
    0x10BD,
    0x10BE,
    0x10BF,
    0x10C0,
    0x10C1,
    0x10C2,
    0x10C3,
    0x10C4,
    0x10C5,
    0x10C7,
    0x10CD,
    0x13F8,
    0x13F9,
    0x13FA,
    0x13FB,
    0x13FC,
    0x13FD,
    0x1C80,
    0x1C81,
    0x1C82,
    0x1C83,
    0x1C84,
    0x1C85,
    0x1C86,
    0x1C87,
    0x1C88,
    0x1C90,
    0x1C91,
    0x1C92,
    0x1C93,
    0x1C94,
    0x1C95,
    0x1C96,
    0x1C97,
    0x1C98,
    0x1C99,
    0x1C9A,
    0x1C9B,
    0x1C9C,
    0x1C9D,
    0x1C9E,
    0x1C9F,
    0x1CA0,
    0x1CA1,
    0x1CA2,
    0x1CA3,
    0x1CA4,
    0x1CA5,
    0x1CA6,
    0x1CA7,
    0x1CA8,
    0x1CA9,
    0x1CAA,
    0x1CAB,
    0x1CAC,
    0x1CAD,
    0x1CAE,
    0x1CAF,
    0x1CB0,
    0x1CB1,
    0x1CB2,
    0x1CB3,
    0x1CB4,
    0x1CB5,
    0x1CB6,
    0x1CB7,
    0x1CB8,
    0x1CB9,
    0x1CBA,
    0x1CBD,
    0x1CBE,
    0x1CBF,
    0x1E00,
    0x1E02,
    0x1E04,
    0x1E06,
    0x1E08,
    0x1E0A,
    0x1E0C,
    0x1E0E,
    0x1E10,
    0x1E12,
    0x1E14,
    0x1E16,
    0x1E18,
    0x1E1A,
    0x1E1C,
    0x1E1E,
    0x1E20,
    0x1E22,
    0x1E24,
    0x1E26,
    0x1E28,
    0x1E2A,
    0x1E2C,
    0x1E2E,
    0x1E30,
    0x1E32,
    0x1E34,
    0x1E36,
    0x1E38,
    0x1E3A,
    0x1E3C,
    0x1E3E,
    0x1E40,
    0x1E42,
    0x1E44,
    0x1E46,
    0x1E48,
    0x1E4A,
    0x1E4C,
    0x1E4E,
    0x1E50,
    0x1E52,
    0x1E54,
    0x1E56,
    0x1E58,
    0x1E5A,
    0x1E5C,
    0x1E5E,
    0x1E60,
    0x1E62,
    0x1E64,
    0x1E66,
    0x1E68,
    0x1E6A,
    0x1E6C,
    0x1E6E,
    0x1E70,
    0x1E72,
    0x1E74,
    0x1E76,
    0x1E78,
    0x1E7A,
    0x1E7C,
    0x1E7E,
    0x1E80,
    0x1E82,
    0x1E84,
    0x1E86,
    0x1E88,
    0x1E8A,
    0x1E8C,
    0x1E8E,
    0x1E90,
    0x1E92,
    0x1E94,
    0x1E9B,
    0x1E9E,
    0x1EA0,
    0x1EA2,
    0x1EA4,
    0x1EA6,
    0x1EA8,
    0x1EAA,
    0x1EAC,
    0x1EAE,
    0x1EB0,
    0x1EB2,
    0x1EB4,
    0x1EB6,
    0x1EB8,
    0x1EBA,
    0x1EBC,
    0x1EBE,
    0x1EC0,
    0x1EC2,
    0x1EC4,
    0x1EC6,
    0x1EC8,
    0x1ECA,
    0x1ECC,
    0x1ECE,
    0x1ED0,
    0x1ED2,
    0x1ED4,
    0x1ED6,
    0x1ED8,
    0x1EDA,
    0x1EDC,
    0x1EDE,
    0x1EE0,
    0x1EE2,
    0x1EE4,
    0x1EE6,
    0x1EE8,
    0x1EEA,
    0x1EEC,
    0x1EEE,
    0x1EF0,
    0x1EF2,
    0x1EF4,
    0x1EF6,
    0x1EF8,
    0x1EFA,
    0x1EFC,
    0x1EFE,
    0x1F08,
    0x1F09,
    0x1F0A,
    0x1F0B,
    0x1F0C,
    0x1F0D,
    0x1F0E,
    0x1F0F,
    0x1F18,
    0x1F19,
    0x1F1A,
    0x1F1B,
    0x1F1C,
    0x1F1D,
    0x1F28,
    0x1F29,
    0x1F2A,
    0x1F2B,
    0x1F2C,
    0x1F2D,
    0x1F2E,
    0x1F2F,
    0x1F38,
    0x1F39,
    0x1F3A,
    0x1F3B,
    0x1F3C,
    0x1F3D,
    0x1F3E,
    0x1F3F,
    0x1F48,
    0x1F49,
    0x1F4A,
    0x1F4B,
    0x1F4C,
    0x1F4D,
    0x1F59,
    0x1F5B,
    0x1F5D,
    0x1F5F,
    0x1F68,
    0x1F69,
    0x1F6A,
    0x1F6B,
    0x1F6C,
    0x1F6D,
    0x1F6E,
    0x1F6F,
    0x1F88,
    0x1F89,
    0x1F8A,
    0x1F8B,
    0x1F8C,
    0x1F8D,
    0x1F8E,
    0x1F8F,
    0x1F98,
    0x1F99,
    0x1F9A,
    0x1F9B,
    0x1F9C,
    0x1F9D,
    0x1F9E,
    0x1F9F,
    0x1FA8,
    0x1FA9,
    0x1FAA,
    0x1FAB,
    0x1FAC,
    0x1FAD,
    0x1FAE,
    0x1FAF,
    0x1FB8,
    0x1FB9,
    0x1FBA,
    0x1FBB,
    0x1FBC,
    0x1FBE,
    0x1FC8,
    0x1FC9,
    0x1FCA,
    0x1FCB,
    0x1FCC,
    0x1FD8,
    0x1FD9,
    0x1FDA,
    0x1FDB,
    0x1FE8,
    0x1FE9,
    0x1FEA,
    0x1FEB,
    0x1FEC,
    0x1FF8,
    0x1FF9,
    0x1FFA,
    0x1FFB,
    0x1FFC,
    0x2126,
    0x212A,
    0x212B,
    0x2132,
    0x2160,
    0x2161,
    0x2162,
    0x2163,
    0x2164,
    0x2165,
    0x2166,
    0x2167,
    0x2168,
    0x2169,
    0x216A,
    0x216B,
    0x216C,
    0x216D,
    0x216E,
    0x216F,
    0x2183,
    0x24B6,
    0x24B7,
    0x24B8,
    0x24B9,
    0x24BA,
    0x24BB,
    0x24BC,
    0x24BD,
    0x24BE,
    0x24BF,
    0x24C0,
    0x24C1,
    0x24C2,
    0x24C3,
    0x24C4,
    0x24C5,
    0x24C6,
    0x24C7,
    0x24C8,
    0x24C9,
    0x24CA,
    0x24CB,
    0x24CC,
    0x24CD,
    0x24CE,
    0x24CF,
    0x2C00,
    0x2C01,
    0x2C02,
    0x2C03,
    0x2C04,
    0x2C05,
    0x2C06,
    0x2C07,
    0x2C08,
    0x2C09,
    0x2C0A,
    0x2C0B,
    0x2C0C,
    0x2C0D,
    0x2C0E,
    0x2C0F,
    0x2C10,
    0x2C11,
    0x2C12,
    0x2C13,
    0x2C14,
    0x2C15,
    0x2C16,
    0x2C17,
    0x2C18,
    0x2C19,
    0x2C1A,
    0x2C1B,
    0x2C1C,
    0x2C1D,
    0x2C1E,
    0x2C1F,
    0x2C20,
    0x2C21,
    0x2C22,
    0x2C23,
    0x2C24,
    0x2C25,
    0x2C26,
    0x2C27,
    0x2C28,
    0x2C29,
    0x2C2A,
    0x2C2B,
    0x2C2C,
    0x2C2D,
    0x2C2E,
    0x2C2F,
    0x2C60,
    0x2C62,
    0x2C63,
    0x2C64,
    0x2C67,
    0x2C69,
    0x2C6B,
    0x2C6D,
    0x2C6E,
    0x2C6F,
    0x2C70,
    0x2C72,
    0x2C75,
    0x2C7E,
    0x2C7F,
    0x2C80,
    0x2C82,
    0x2C84,
    0x2C86,
    0x2C88,
    0x2C8A,
    0x2C8C,
    0x2C8E,
    0x2C90,
    0x2C92,
    0x2C94,
    0x2C96,
    0x2C98,
    0x2C9A,
    0x2C9C,
    0x2C9E,
    0x2CA0,
    0x2CA2,
    0x2CA4,
    0x2CA6,
    0x2CA8,
    0x2CAA,
    0x2CAC,
    0x2CAE,
    0x2CB0,
    0x2CB2,
    0x2CB4,
    0x2CB6,
    0x2CB8,
    0x2CBA,
    0x2CBC,
    0x2CBE,
    0x2CC0,
    0x2CC2,
    0x2CC4,
    0x2CC6,
    0x2CC8,
    0x2CCA,
    0x2CCC,
    0x2CCE,
    0x2CD0,
    0x2CD2,
    0x2CD4,
    0x2CD6,
    0x2CD8,
    0x2CDA,
    0x2CDC,
    0x2CDE,
    0x2CE0,
    0x2CE2,
    0x2CEB,
    0x2CED,
    0x2CF2,
    0xA640,
    0xA642,
    0xA644,
    0xA646,
    0xA648,
    0xA64A,
    0xA64C,
    0xA64E,
    0xA650,
    0xA652,
    0xA654,
    0xA656,
    0xA658,
    0xA65A,
    0xA65C,
    0xA65E,
    0xA660,
    0xA662,
    0xA664,
    0xA666,
    0xA668,
    0xA66A,
    0xA66C,
    0xA680,
    0xA682,
    0xA684,
    0xA686,
    0xA688,
    0xA68A,
    0xA68C,
    0xA68E,
    0xA690,
    0xA692,
    0xA694,
    0xA696,
    0xA698,
    0xA69A,
    0xA722,
    0xA724,
    0xA726,
    0xA728,
    0xA72A,
    0xA72C,
    0xA72E,
    0xA732,
    0xA734,
    0xA736,
    0xA738,
    0xA73A,
    0xA73C,
    0xA73E,
    0xA740,
    0xA742,
    0xA744,
    0xA746,
    0xA748,
    0xA74A,
    0xA74C,
    0xA74E,
    0xA750,
    0xA752,
    0xA754,
    0xA756,
    0xA758,
    0xA75A,
    0xA75C,
    0xA75E,
    0xA760,
    0xA762,
    0xA764,
    0xA766,
    0xA768,
    0xA76A,
    0xA76C,
    0xA76E,
    0xA779,
    0xA77B,
    0xA77D,
    0xA77E,
    0xA780,
    0xA782,
    0xA784,
    0xA786,
    0xA78B,
    0xA78D,
    0xA790,
    0xA792,
    0xA796,
    0xA798,
    0xA79A,
    0xA79C,
    0xA79E,
    0xA7A0,
    0xA7A2,
    0xA7A4,
    0xA7A6,
    0xA7A8,
    0xA7AA,
    0xA7AB,
    0xA7AC,
    0xA7AD,
    0xA7AE,
    0xA7B0,
    0xA7B1,
    0xA7B2,
    0xA7B3,
    0xA7B4,
    0xA7B6,
    0xA7B8,
    0xA7BA,
    0xA7BC,
    0xA7BE,
    0xA7C0,
    0xA7C2,
    0xA7C4,
    0xA7C5,
    0xA7C6,
    0xA7C7,
    0xA7C9,
    0xA7D0,
    0xA7D6,
    0xA7D8,
    0xA7F5,
    0xAB70,
    0xAB71,
    0xAB72,
    0xAB73,
    0xAB74,
    0xAB75,
    0xAB76,
    0xAB77,
    0xAB78,
    0xAB79,
    0xAB7A,
    0xAB7B,
    0xAB7C,
    0xAB7D,
    0xAB7E,
    0xAB7F,
    0xAB80,
    0xAB81,
    0xAB82,
    0xAB83,
    0xAB84,
    0xAB85,
    0xAB86,
    0xAB87,
    0xAB88,
    0xAB89,
    0xAB8A,
    0xAB8B,
    0xAB8C,
    0xAB8D,
    0xAB8E,
    0xAB8F,
    0xAB90,
    0xAB91,
    0xAB92,
    0xAB93,
    0xAB94,
    0xAB95,
    0xAB96,
    0xAB97,
    0xAB98,
    0xAB99,
    0xAB9A,
    0xAB9B,
    0xAB9C,
    0xAB9D,
    0xAB9E,
    0xAB9F,
    0xABA0,
    0xABA1,
    0xABA2,
    0xABA3,
    0xABA4,
    0xABA5,
    0xABA6,
    0xABA7,
    0xABA8,
    0xABA9,
    0xABAA,
    0xABAB,
    0xABAC,
    0xABAD,
    0xABAE,
    0xABAF,
    0xABB0,
    0xABB1,
    0xABB2,
    0xABB3,
    0xABB4,
    0xABB5,
    0xABB6,
    0xABB7,
    0xABB8,
    0xABB9,
    0xABBA,
    0xABBB,
    0xABBC,
    0xABBD,
    0xABBE,
    0xABBF,
    0xFF21,
    0xFF22,
    0xFF23,
    0xFF24,
    0xFF25,
    0xFF26,
    0xFF27,
    0xFF28,
    0xFF29,
    0xFF2A,
    0xFF2B,
    0xFF2C,
    0xFF2D,
    0xFF2E,
    0xFF2F,
    0xFF30,
    0xFF31,
    0xFF32,
    0xFF33,
    0xFF34,
    0xFF35,
    0xFF36,
    0xFF37,
    0xFF38,
    0xFF39,
    0xFF3A,
    0x10400,
    0x10401,
    0x10402,
    0x10403,
    0x10404,
    0x10405,
    0x10406,
    0x10407,
    0x10408,
    0x10409,
    0x1040A,
    0x1040B,
    0x1040C,
    0x1040D,
    0x1040E,
    0x1040F,
    0x10410,
    0x10411,
    0x10412,
    0x10413,
    0x10414,
    0x10415,
    0x10416,
    0x10417,
    0x10418,
    0x10419,
    0x1041A,
    0x1041B,
    0x1041C,
    0x1041D,
    0x1041E,
    0x1041F,
    0x10420,
    0x10421,
    0x10422,
    0x10423,
    0x10424,
    0x10425,
    0x10426,
    0x10427,
    0x104B0,
    0x104B1,
    0x104B2,
    0x104B3,
    0x104B4,
    0x104B5,
    0x104B6,
    0x104B7,
    0x104B8,
    0x104B9,
    0x104BA,
    0x104BB,
    0x104BC,
    0x104BD,
    0x104BE,
    0x104BF,
    0x104C0,
    0x104C1,
    0x104C2,
    0x104C3,
    0x104C4,
    0x104C5,
    0x104C6,
    0x104C7,
    0x104C8,
    0x104C9,
    0x104CA,
    0x104CB,
    0x104CC,
    0x104CD,
    0x104CE,
    0x104CF,
    0x104D0,
    0x104D1,
    0x104D2,
    0x104D3,
    0x10570,
    0x10571,
    0x10572,
    0x10573,
    0x10574,
    0x10575,
    0x10576,
    0x10577,
    0x10578,
    0x10579,
    0x1057A,
    0x1057C,
    0x1057D,
    0x1057E,
    0x1057F,
    0x10580,
    0x10581,
    0x10582,
    0x10583,
    0x10584,
    0x10585,
    0x10586,
    0x10587,
    0x10588,
    0x10589,
    0x1058A,
    0x1058C,
    0x1058D,
    0x1058E,
    0x1058F,
    0x10590,
    0x10591,
    0x10592,
    0x10594,
    0x10595,
    0x10C80,
    0x10C81,
    0x10C82,
    0x10C83,
    0x10C84,
    0x10C85,
    0x10C86,
    0x10C87,
    0x10C88,
    0x10C89,
    0x10C8A,
    0x10C8B,
    0x10C8C,
    0x10C8D,
    0x10C8E,
    0x10C8F,
    0x10C90,
    0x10C91,
    0x10C92,
    0x10C93,
    0x10C94,
    0x10C95,
    0x10C96,
    0x10C97,
    0x10C98,
    0x10C99,
    0x10C9A,
    0x10C9B,
    0x10C9C,
    0x10C9D,
    0x10C9E,
    0x10C9F,
    0x10CA0,
    0x10CA1,
    0x10CA2,
    0x10CA3,
    0x10CA4,
    0x10CA5,
    0x10CA6,
    0x10CA7,
    0x10CA8,
    0x10CA9,
    0x10CAA,
    0x10CAB,
    0x10CAC,
    0x10CAD,
    0x10CAE,
    0x10CAF,
    0x10CB0,
    0x10CB1,
    0x10CB2,
    0x118A0,
    0x118A1,
    0x118A2,
    0x118A3,
    0x118A4,
    0x118A5,
    0x118A6,
    0x118A7,
    0x118A8,
    0x118A9,
    0x118AA,
    0x118AB,
    0x118AC,
    0x118AD,
    0x118AE,
    0x118AF,
    0x118B0,
    0x118B1,
    0x118B2,
    0x118B3,
    0x118B4,
    0x118B5,
    0x118B6,
    0x118B7,
    0x118B8,
    0x118B9,
    0x118BA,
    0x118BB,
    0x118BC,
    0x118BD,
    0x118BE,
    0x118BF,
    0x16E40,
    0x16E41,
    0x16E42,
    0x16E43,
    0x16E44,
    0x16E45,
    0x16E46,
    0x16E47,
    0x16E48,
    0x16E49,
    0x16E4A,
    0x16E4B,
    0x16E4C,
    0x16E4D,
    0x16E4E,
    0x16E4F,
    0x16E50,
    0x16E51,
    0x16E52,
    0x16E53,
    0x16E54,
    0x16E55,
    0x16E56,
    0x16E57,
    0x16E58,
    0x16E59,
    0x16E5A,
    0x16E5B,
    0x16E5C,
    0x16E5D,
    0x16E5E,
    0x16E5F,
    0x1E900,
    0x1E901,
    0x1E902,
    0x1E903,
    0x1E904,
    0x1E905,
    0x1E906,
    0x1E907,
    0x1E908,
    0x1E909,
    0x1E90A,
    0x1E90B,
    0x1E90C,
    0x1E90D,
    0x1E90E,
    0x1E90F,
    0x1E910,
    0x1E911,
    0x1E912,
    0x1E913,
    0x1E914,
    0x1E915,
    0x1E916,
    0x1E917,
    0x1E918,
    0x1E919,
    0x1E91A,
    0x1E91B,
    0x1E91C,
    0x1E91D,
    0x1E91E,
    0x1E91F,
    0x1E920,
    0x1E921,
    0x00000
};
static const uint32_t STRING_LOWERCASES[] = {
    /*0x61,
    0x62,
    0x63,
    0x64,
    0x65,
    0x66,
    0x67,
    0x68,
    0x69,
    0x6a,
    0x6b,
    0x6c,
    0x6d,
    0x6e,
    0x6f,
    0x70,
    0x71,
    0x72,
    0x73,
    0x74,
    0x75,
    0x76,
    0x77,
    0x78,
    0x79,
    0x7a,*/
    0x3bc,
    0xe0,
    0xe1,
    0xe2,
    0xe3,
    0xe4,
    0xe5,
    0xe6,
    0xe7,
    0xe8,
    0xe9,
    0xea,
    0xeb,
    0xec,
    0xed,
    0xee,
    0xef,
    0xf0,
    0xf1,
    0xf2,
    0xf3,
    0xf4,
    0xf5,
    0xf6,
    0xf8,
    0xf9,
    0xfa,
    0xfb,
    0xfc,
    0xfd,
    0xfe,
    0x101,
    0x103,
    0x105,
    0x107,
    0x109,
    0x10b,
    0x10d,
    0x10f,
    0x111,
    0x113,
    0x115,
    0x117,
    0x119,
    0x11b,
    0x11d,
    0x11f,
    0x121,
    0x123,
    0x125,
    0x127,
    0x129,
    0x12b,
    0x12d,
    0x12f,
    0x133,
    0x135,
    0x137,
    0x13a,
    0x13c,
    0x13e,
    0x140,
    0x142,
    0x144,
    0x146,
    0x148,
    0x14b,
    0x14d,
    0x14f,
    0x151,
    0x153,
    0x155,
    0x157,
    0x159,
    0x15b,
    0x15d,
    0x15f,
    0x161,
    0x163,
    0x165,
    0x167,
    0x169,
    0x16b,
    0x16d,
    0x16f,
    0x171,
    0x173,
    0x175,
    0x177,
    0xff,
    0x17a,
    0x17c,
    0x17e,
    0x73,
    0x253,
    0x183,
    0x185,
    0x254,
    0x188,
    0x256,
    0x257,
    0x18c,
    0x1dd,
    0x259,
    0x25b,
    0x192,
    0x260,
    0x263,
    0x269,
    0x268,
    0x199,
    0x26f,
    0x272,
    0x275,
    0x1a1,
    0x1a3,
    0x1a5,
    0x280,
    0x1a8,
    0x283,
    0x1ad,
    0x288,
    0x1b0,
    0x28a,
    0x28b,
    0x1b4,
    0x1b6,
    0x292,
    0x1b9,
    0x1bd,
    0x1c6,
    0x1c6,
    0x1c9,
    0x1c9,
    0x1cc,
    0x1cc,
    0x1ce,
    0x1d0,
    0x1d2,
    0x1d4,
    0x1d6,
    0x1d8,
    0x1da,
    0x1dc,
    0x1df,
    0x1e1,
    0x1e3,
    0x1e5,
    0x1e7,
    0x1e9,
    0x1eb,
    0x1ed,
    0x1ef,
    0x1f3,
    0x1f3,
    0x1f5,
    0x195,
    0x1bf,
    0x1f9,
    0x1fb,
    0x1fd,
    0x1ff,
    0x201,
    0x203,
    0x205,
    0x207,
    0x209,
    0x20b,
    0x20d,
    0x20f,
    0x211,
    0x213,
    0x215,
    0x217,
    0x219,
    0x21b,
    0x21d,
    0x21f,
    0x19e,
    0x223,
    0x225,
    0x227,
    0x229,
    0x22b,
    0x22d,
    0x22f,
    0x231,
    0x233,
    0x2c65,
    0x23c,
    0x19a,
    0x2c66,
    0x242,
    0x180,
    0x289,
    0x28c,
    0x247,
    0x249,
    0x24b,
    0x24d,
    0x24f,
    0x3b9,
    0x371,
    0x373,
    0x377,
    0x3f3,
    0x3ac,
    0x3ad,
    0x3ae,
    0x3af,
    0x3cc,
    0x3cd,
    0x3ce,
    0x3b1,
    0x3b2,
    0x3b3,
    0x3b4,
    0x3b5,
    0x3b6,
    0x3b7,
    0x3b8,
    0x3b9,
    0x3ba,
    0x3bb,
    0x3bc,
    0x3bd,
    0x3be,
    0x3bf,
    0x3c0,
    0x3c1,
    0x3c3,
    0x3c4,
    0x3c5,
    0x3c6,
    0x3c7,
    0x3c8,
    0x3c9,
    0x3ca,
    0x3cb,
    0x3c3,
    0x3d7,
    0x3b2,
    0x3b8,
    0x3c6,
    0x3c0,
    0x3d9,
    0x3db,
    0x3dd,
    0x3df,
    0x3e1,
    0x3e3,
    0x3e5,
    0x3e7,
    0x3e9,
    0x3eb,
    0x3ed,
    0x3ef,
    0x3ba,
    0x3c1,
    0x3b8,
    0x3b5,
    0x3f8,
    0x3f2,
    0x3fb,
    0x37b,
    0x37c,
    0x37d,
    0x450,
    0x451,
    0x452,
    0x453,
    0x454,
    0x455,
    0x456,
    0x457,
    0x458,
    0x459,
    0x45a,
    0x45b,
    0x45c,
    0x45d,
    0x45e,
    0x45f,
    0x430,
    0x431,
    0x432,
    0x433,
    0x434,
    0x435,
    0x436,
    0x437,
    0x438,
    0x439,
    0x43a,
    0x43b,
    0x43c,
    0x43d,
    0x43e,
    0x43f,
    0x440,
    0x441,
    0x442,
    0x443,
    0x444,
    0x445,
    0x446,
    0x447,
    0x448,
    0x449,
    0x44a,
    0x44b,
    0x44c,
    0x44d,
    0x44e,
    0x44f,
    0x461,
    0x463,
    0x465,
    0x467,
    0x469,
    0x46b,
    0x46d,
    0x46f,
    0x471,
    0x473,
    0x475,
    0x477,
    0x479,
    0x47b,
    0x47d,
    0x47f,
    0x481,
    0x48b,
    0x48d,
    0x48f,
    0x491,
    0x493,
    0x495,
    0x497,
    0x499,
    0x49b,
    0x49d,
    0x49f,
    0x4a1,
    0x4a3,
    0x4a5,
    0x4a7,
    0x4a9,
    0x4ab,
    0x4ad,
    0x4af,
    0x4b1,
    0x4b3,
    0x4b5,
    0x4b7,
    0x4b9,
    0x4bb,
    0x4bd,
    0x4bf,
    0x4cf,
    0x4c2,
    0x4c4,
    0x4c6,
    0x4c8,
    0x4ca,
    0x4cc,
    0x4ce,
    0x4d1,
    0x4d3,
    0x4d5,
    0x4d7,
    0x4d9,
    0x4db,
    0x4dd,
    0x4df,
    0x4e1,
    0x4e3,
    0x4e5,
    0x4e7,
    0x4e9,
    0x4eb,
    0x4ed,
    0x4ef,
    0x4f1,
    0x4f3,
    0x4f5,
    0x4f7,
    0x4f9,
    0x4fb,
    0x4fd,
    0x4ff,
    0x501,
    0x503,
    0x505,
    0x507,
    0x509,
    0x50b,
    0x50d,
    0x50f,
    0x511,
    0x513,
    0x515,
    0x517,
    0x519,
    0x51b,
    0x51d,
    0x51f,
    0x521,
    0x523,
    0x525,
    0x527,
    0x529,
    0x52b,
    0x52d,
    0x52f,
    0x561,
    0x562,
    0x563,
    0x564,
    0x565,
    0x566,
    0x567,
    0x568,
    0x569,
    0x56a,
    0x56b,
    0x56c,
    0x56d,
    0x56e,
    0x56f,
    0x570,
    0x571,
    0x572,
    0x573,
    0x574,
    0x575,
    0x576,
    0x577,
    0x578,
    0x579,
    0x57a,
    0x57b,
    0x57c,
    0x57d,
    0x57e,
    0x57f,
    0x580,
    0x581,
    0x582,
    0x583,
    0x584,
    0x585,
    0x586,
    0x2d00,
    0x2d01,
    0x2d02,
    0x2d03,
    0x2d04,
    0x2d05,
    0x2d06,
    0x2d07,
    0x2d08,
    0x2d09,
    0x2d0a,
    0x2d0b,
    0x2d0c,
    0x2d0d,
    0x2d0e,
    0x2d0f,
    0x2d10,
    0x2d11,
    0x2d12,
    0x2d13,
    0x2d14,
    0x2d15,
    0x2d16,
    0x2d17,
    0x2d18,
    0x2d19,
    0x2d1a,
    0x2d1b,
    0x2d1c,
    0x2d1d,
    0x2d1e,
    0x2d1f,
    0x2d20,
    0x2d21,
    0x2d22,
    0x2d23,
    0x2d24,
    0x2d25,
    0x2d27,
    0x2d2d,
    0x13f0,
    0x13f1,
    0x13f2,
    0x13f3,
    0x13f4,
    0x13f5,
    0x432,
    0x434,
    0x43e,
    0x441,
    0x442,
    0x442,
    0x44a,
    0x463,
    0xa64b,
    0x10d0,
    0x10d1,
    0x10d2,
    0x10d3,
    0x10d4,
    0x10d5,
    0x10d6,
    0x10d7,
    0x10d8,
    0x10d9,
    0x10da,
    0x10db,
    0x10dc,
    0x10dd,
    0x10de,
    0x10df,
    0x10e0,
    0x10e1,
    0x10e2,
    0x10e3,
    0x10e4,
    0x10e5,
    0x10e6,
    0x10e7,
    0x10e8,
    0x10e9,
    0x10ea,
    0x10eb,
    0x10ec,
    0x10ed,
    0x10ee,
    0x10ef,
    0x10f0,
    0x10f1,
    0x10f2,
    0x10f3,
    0x10f4,
    0x10f5,
    0x10f6,
    0x10f7,
    0x10f8,
    0x10f9,
    0x10fa,
    0x10fd,
    0x10fe,
    0x10ff,
    0x1e01,
    0x1e03,
    0x1e05,
    0x1e07,
    0x1e09,
    0x1e0b,
    0x1e0d,
    0x1e0f,
    0x1e11,
    0x1e13,
    0x1e15,
    0x1e17,
    0x1e19,
    0x1e1b,
    0x1e1d,
    0x1e1f,
    0x1e21,
    0x1e23,
    0x1e25,
    0x1e27,
    0x1e29,
    0x1e2b,
    0x1e2d,
    0x1e2f,
    0x1e31,
    0x1e33,
    0x1e35,
    0x1e37,
    0x1e39,
    0x1e3b,
    0x1e3d,
    0x1e3f,
    0x1e41,
    0x1e43,
    0x1e45,
    0x1e47,
    0x1e49,
    0x1e4b,
    0x1e4d,
    0x1e4f,
    0x1e51,
    0x1e53,
    0x1e55,
    0x1e57,
    0x1e59,
    0x1e5b,
    0x1e5d,
    0x1e5f,
    0x1e61,
    0x1e63,
    0x1e65,
    0x1e67,
    0x1e69,
    0x1e6b,
    0x1e6d,
    0x1e6f,
    0x1e71,
    0x1e73,
    0x1e75,
    0x1e77,
    0x1e79,
    0x1e7b,
    0x1e7d,
    0x1e7f,
    0x1e81,
    0x1e83,
    0x1e85,
    0x1e87,
    0x1e89,
    0x1e8b,
    0x1e8d,
    0x1e8f,
    0x1e91,
    0x1e93,
    0x1e95,
    0x1e61,
    0xdf,
    0x1ea1,
    0x1ea3,
    0x1ea5,
    0x1ea7,
    0x1ea9,
    0x1eab,
    0x1ead,
    0x1eaf,
    0x1eb1,
    0x1eb3,
    0x1eb5,
    0x1eb7,
    0x1eb9,
    0x1ebb,
    0x1ebd,
    0x1ebf,
    0x1ec1,
    0x1ec3,
    0x1ec5,
    0x1ec7,
    0x1ec9,
    0x1ecb,
    0x1ecd,
    0x1ecf,
    0x1ed1,
    0x1ed3,
    0x1ed5,
    0x1ed7,
    0x1ed9,
    0x1edb,
    0x1edd,
    0x1edf,
    0x1ee1,
    0x1ee3,
    0x1ee5,
    0x1ee7,
    0x1ee9,
    0x1eeb,
    0x1eed,
    0x1eef,
    0x1ef1,
    0x1ef3,
    0x1ef5,
    0x1ef7,
    0x1ef9,
    0x1efb,
    0x1efd,
    0x1eff,
    0x1f00,
    0x1f01,
    0x1f02,
    0x1f03,
    0x1f04,
    0x1f05,
    0x1f06,
    0x1f07,
    0x1f10,
    0x1f11,
    0x1f12,
    0x1f13,
    0x1f14,
    0x1f15,
    0x1f20,
    0x1f21,
    0x1f22,
    0x1f23,
    0x1f24,
    0x1f25,
    0x1f26,
    0x1f27,
    0x1f30,
    0x1f31,
    0x1f32,
    0x1f33,
    0x1f34,
    0x1f35,
    0x1f36,
    0x1f37,
    0x1f40,
    0x1f41,
    0x1f42,
    0x1f43,
    0x1f44,
    0x1f45,
    0x1f51,
    0x1f53,
    0x1f55,
    0x1f57,
    0x1f60,
    0x1f61,
    0x1f62,
    0x1f63,
    0x1f64,
    0x1f65,
    0x1f66,
    0x1f67,
    0x1f80,
    0x1f81,
    0x1f82,
    0x1f83,
    0x1f84,
    0x1f85,
    0x1f86,
    0x1f87,
    0x1f90,
    0x1f91,
    0x1f92,
    0x1f93,
    0x1f94,
    0x1f95,
    0x1f96,
    0x1f97,
    0x1fa0,
    0x1fa1,
    0x1fa2,
    0x1fa3,
    0x1fa4,
    0x1fa5,
    0x1fa6,
    0x1fa7,
    0x1fb0,
    0x1fb1,
    0x1f70,
    0x1f71,
    0x1fb3,
    0x3b9,
    0x1f72,
    0x1f73,
    0x1f74,
    0x1f75,
    0x1fc3,
    0x1fd0,
    0x1fd1,
    0x1f76,
    0x1f77,
    0x1fe0,
    0x1fe1,
    0x1f7a,
    0x1f7b,
    0x1fe5,
    0x1f78,
    0x1f79,
    0x1f7c,
    0x1f7d,
    0x1ff3,
    0x3c9,
    0x6b,
    0xe5,
    0x214e,
    0x2170,
    0x2171,
    0x2172,
    0x2173,
    0x2174,
    0x2175,
    0x2176,
    0x2177,
    0x2178,
    0x2179,
    0x217a,
    0x217b,
    0x217c,
    0x217d,
    0x217e,
    0x217f,
    0x2184,
    0x24d0,
    0x24d1,
    0x24d2,
    0x24d3,
    0x24d4,
    0x24d5,
    0x24d6,
    0x24d7,
    0x24d8,
    0x24d9,
    0x24da,
    0x24db,
    0x24dc,
    0x24dd,
    0x24de,
    0x24df,
    0x24e0,
    0x24e1,
    0x24e2,
    0x24e3,
    0x24e4,
    0x24e5,
    0x24e6,
    0x24e7,
    0x24e8,
    0x24e9,
    0x2c30,
    0x2c31,
    0x2c32,
    0x2c33,
    0x2c34,
    0x2c35,
    0x2c36,
    0x2c37,
    0x2c38,
    0x2c39,
    0x2c3a,
    0x2c3b,
    0x2c3c,
    0x2c3d,
    0x2c3e,
    0x2c3f,
    0x2c40,
    0x2c41,
    0x2c42,
    0x2c43,
    0x2c44,
    0x2c45,
    0x2c46,
    0x2c47,
    0x2c48,
    0x2c49,
    0x2c4a,
    0x2c4b,
    0x2c4c,
    0x2c4d,
    0x2c4e,
    0x2c4f,
    0x2c50,
    0x2c51,
    0x2c52,
    0x2c53,
    0x2c54,
    0x2c55,
    0x2c56,
    0x2c57,
    0x2c58,
    0x2c59,
    0x2c5a,
    0x2c5b,
    0x2c5c,
    0x2c5d,
    0x2c5e,
    0x2c5f,
    0x2c61,
    0x26b,
    0x1d7d,
    0x27d,
    0x2c68,
    0x2c6a,
    0x2c6c,
    0x251,
    0x271,
    0x250,
    0x252,
    0x2c73,
    0x2c76,
    0x23f,
    0x240,
    0x2c81,
    0x2c83,
    0x2c85,
    0x2c87,
    0x2c89,
    0x2c8b,
    0x2c8d,
    0x2c8f,
    0x2c91,
    0x2c93,
    0x2c95,
    0x2c97,
    0x2c99,
    0x2c9b,
    0x2c9d,
    0x2c9f,
    0x2ca1,
    0x2ca3,
    0x2ca5,
    0x2ca7,
    0x2ca9,
    0x2cab,
    0x2cad,
    0x2caf,
    0x2cb1,
    0x2cb3,
    0x2cb5,
    0x2cb7,
    0x2cb9,
    0x2cbb,
    0x2cbd,
    0x2cbf,
    0x2cc1,
    0x2cc3,
    0x2cc5,
    0x2cc7,
    0x2cc9,
    0x2ccb,
    0x2ccd,
    0x2ccf,
    0x2cd1,
    0x2cd3,
    0x2cd5,
    0x2cd7,
    0x2cd9,
    0x2cdb,
    0x2cdd,
    0x2cdf,
    0x2ce1,
    0x2ce3,
    0x2cec,
    0x2cee,
    0x2cf3,
    0xa641,
    0xa643,
    0xa645,
    0xa647,
    0xa649,
    0xa64b,
    0xa64d,
    0xa64f,
    0xa651,
    0xa653,
    0xa655,
    0xa657,
    0xa659,
    0xa65b,
    0xa65d,
    0xa65f,
    0xa661,
    0xa663,
    0xa665,
    0xa667,
    0xa669,
    0xa66b,
    0xa66d,
    0xa681,
    0xa683,
    0xa685,
    0xa687,
    0xa689,
    0xa68b,
    0xa68d,
    0xa68f,
    0xa691,
    0xa693,
    0xa695,
    0xa697,
    0xa699,
    0xa69b,
    0xa723,
    0xa725,
    0xa727,
    0xa729,
    0xa72b,
    0xa72d,
    0xa72f,
    0xa733,
    0xa735,
    0xa737,
    0xa739,
    0xa73b,
    0xa73d,
    0xa73f,
    0xa741,
    0xa743,
    0xa745,
    0xa747,
    0xa749,
    0xa74b,
    0xa74d,
    0xa74f,
    0xa751,
    0xa753,
    0xa755,
    0xa757,
    0xa759,
    0xa75b,
    0xa75d,
    0xa75f,
    0xa761,
    0xa763,
    0xa765,
    0xa767,
    0xa769,
    0xa76b,
    0xa76d,
    0xa76f,
    0xa77a,
    0xa77c,
    0x1d79,
    0xa77f,
    0xa781,
    0xa783,
    0xa785,
    0xa787,
    0xa78c,
    0x265,
    0xa791,
    0xa793,
    0xa797,
    0xa799,
    0xa79b,
    0xa79d,
    0xa79f,
    0xa7a1,
    0xa7a3,
    0xa7a5,
    0xa7a7,
    0xa7a9,
    0x266,
    0x25c,
    0x261,
    0x26c,
    0x26a,
    0x29e,
    0x287,
    0x29d,
    0xab53,
    0xa7b5,
    0xa7b7,
    0xa7b9,
    0xa7bb,
    0xa7bd,
    0xa7bf,
    0xa7c1,
    0xa7c3,
    0xa794,
    0x282,
    0x1d8e,
    0xa7c8,
    0xa7ca,
    0xa7d1,
    0xa7d7,
    0xa7d9,
    0xa7f6,
    0x13a0,
    0x13a1,
    0x13a2,
    0x13a3,
    0x13a4,
    0x13a5,
    0x13a6,
    0x13a7,
    0x13a8,
    0x13a9,
    0x13aa,
    0x13ab,
    0x13ac,
    0x13ad,
    0x13ae,
    0x13af,
    0x13b0,
    0x13b1,
    0x13b2,
    0x13b3,
    0x13b4,
    0x13b5,
    0x13b6,
    0x13b7,
    0x13b8,
    0x13b9,
    0x13ba,
    0x13bb,
    0x13bc,
    0x13bd,
    0x13be,
    0x13bf,
    0x13c0,
    0x13c1,
    0x13c2,
    0x13c3,
    0x13c4,
    0x13c5,
    0x13c6,
    0x13c7,
    0x13c8,
    0x13c9,
    0x13ca,
    0x13cb,
    0x13cc,
    0x13cd,
    0x13ce,
    0x13cf,
    0x13d0,
    0x13d1,
    0x13d2,
    0x13d3,
    0x13d4,
    0x13d5,
    0x13d6,
    0x13d7,
    0x13d8,
    0x13d9,
    0x13da,
    0x13db,
    0x13dc,
    0x13dd,
    0x13de,
    0x13df,
    0x13e0,
    0x13e1,
    0x13e2,
    0x13e3,
    0x13e4,
    0x13e5,
    0x13e6,
    0x13e7,
    0x13e8,
    0x13e9,
    0x13ea,
    0x13eb,
    0x13ec,
    0x13ed,
    0x13ee,
    0x13ef,
    0xff41,
    0xff42,
    0xff43,
    0xff44,
    0xff45,
    0xff46,
    0xff47,
    0xff48,
    0xff49,
    0xff4a,
    0xff4b,
    0xff4c,
    0xff4d,
    0xff4e,
    0xff4f,
    0xff50,
    0xff51,
    0xff52,
    0xff53,
    0xff54,
    0xff55,
    0xff56,
    0xff57,
    0xff58,
    0xff59,
    0xff5a,
    0x10428,
    0x10429,
    0x1042a,
    0x1042b,
    0x1042c,
    0x1042d,
    0x1042e,
    0x1042f,
    0x10430,
    0x10431,
    0x10432,
    0x10433,
    0x10434,
    0x10435,
    0x10436,
    0x10437,
    0x10438,
    0x10439,
    0x1043a,
    0x1043b,
    0x1043c,
    0x1043d,
    0x1043e,
    0x1043f,
    0x10440,
    0x10441,
    0x10442,
    0x10443,
    0x10444,
    0x10445,
    0x10446,
    0x10447,
    0x10448,
    0x10449,
    0x1044a,
    0x1044b,
    0x1044c,
    0x1044d,
    0x1044e,
    0x1044f,
    0x104d8,
    0x104d9,
    0x104da,
    0x104db,
    0x104dc,
    0x104dd,
    0x104de,
    0x104df,
    0x104e0,
    0x104e1,
    0x104e2,
    0x104e3,
    0x104e4,
    0x104e5,
    0x104e6,
    0x104e7,
    0x104e8,
    0x104e9,
    0x104ea,
    0x104eb,
    0x104ec,
    0x104ed,
    0x104ee,
    0x104ef,
    0x104f0,
    0x104f1,
    0x104f2,
    0x104f3,
    0x104f4,
    0x104f5,
    0x104f6,
    0x104f7,
    0x104f8,
    0x104f9,
    0x104fa,
    0x104fb,
    0x10597,
    0x10598,
    0x10599,
    0x1059a,
    0x1059b,
    0x1059c,
    0x1059d,
    0x1059e,
    0x1059f,
    0x105a0,
    0x105a1,
    0x105a3,
    0x105a4,
    0x105a5,
    0x105a6,
    0x105a7,
    0x105a8,
    0x105a9,
    0x105aa,
    0x105ab,
    0x105ac,
    0x105ad,
    0x105ae,
    0x105af,
    0x105b0,
    0x105b1,
    0x105b3,
    0x105b4,
    0x105b5,
    0x105b6,
    0x105b7,
    0x105b8,
    0x105b9,
    0x105bb,
    0x105bc,
    0x10cc0,
    0x10cc1,
    0x10cc2,
    0x10cc3,
    0x10cc4,
    0x10cc5,
    0x10cc6,
    0x10cc7,
    0x10cc8,
    0x10cc9,
    0x10cca,
    0x10ccb,
    0x10ccc,
    0x10ccd,
    0x10cce,
    0x10ccf,
    0x10cd0,
    0x10cd1,
    0x10cd2,
    0x10cd3,
    0x10cd4,
    0x10cd5,
    0x10cd6,
    0x10cd7,
    0x10cd8,
    0x10cd9,
    0x10cda,
    0x10cdb,
    0x10cdc,
    0x10cdd,
    0x10cde,
    0x10cdf,
    0x10ce0,
    0x10ce1,
    0x10ce2,
    0x10ce3,
    0x10ce4,
    0x10ce5,
    0x10ce6,
    0x10ce7,
    0x10ce8,
    0x10ce9,
    0x10cea,
    0x10ceb,
    0x10cec,
    0x10ced,
    0x10cee,
    0x10cef,
    0x10cf0,
    0x10cf1,
    0x10cf2,
    0x118c0,
    0x118c1,
    0x118c2,
    0x118c3,
    0x118c4,
    0x118c5,
    0x118c6,
    0x118c7,
    0x118c8,
    0x118c9,
    0x118ca,
    0x118cb,
    0x118cc,
    0x118cd,
    0x118ce,
    0x118cf,
    0x118d0,
    0x118d1,
    0x118d2,
    0x118d3,
    0x118d4,
    0x118d5,
    0x118d6,
    0x118d7,
    0x118d8,
    0x118d9,
    0x118da,
    0x118db,
    0x118dc,
    0x118dd,
    0x118de,
    0x118df,
    0x16e60,
    0x16e61,
    0x16e62,
    0x16e63,
    0x16e64,
    0x16e65,
    0x16e66,
    0x16e67,
    0x16e68,
    0x16e69,
    0x16e6a,
    0x16e6b,
    0x16e6c,
    0x16e6d,
    0x16e6e,
    0x16e6f,
    0x16e70,
    0x16e71,
    0x16e72,
    0x16e73,
    0x16e74,
    0x16e75,
    0x16e76,
    0x16e77,
    0x16e78,
    0x16e79,
    0x16e7a,
    0x16e7b,
    0x16e7c,
    0x16e7d,
    0x16e7e,
    0x16e7f,
    0x1e922,
    0x1e923,
    0x1e924,
    0x1e925,
    0x1e926,
    0x1e927,
    0x1e928,
    0x1e929,
    0x1e92a,
    0x1e92b,
    0x1e92c,
    0x1e92d,
    0x1e92e,
    0x1e92f,
    0x1e930,
    0x1e931,
    0x1e932,
    0x1e933,
    0x1e934,
    0x1e935,
    0x1e936,
    0x1e937,
    0x1e938,
    0x1e939,
    0x1e93a,
    0x1e93b,
    0x1e93c,
    0x1e93d,
    0x1e93e,
    0x1e93f,
    0x1e940,
    0x1e941,
    0x1e942,
    0x1e943,
    0x0000
};
static const uint32_t STRING_LOWERCASE_TO_COMPONENTS[][5] = {
    {0x00DF, 0x0073, 0x0073, 0x0000},
    {0x0130, 0x0069, 0x0307, 0x0000},
    {0x0149, 0x02BC, 0x006E, 0x0000},
    {0x01F0, 0x006A, 0x030C, 0x0000},
    {0x0390, 0x03B9, 0x0308, 0x0301, 0x0000},
    {0x03B0, 0x03C5, 0x0308, 0x0301, 0x0000},
    {0x0587, 0x0565, 0x0582, 0x0000},
    {0x1E96, 0x0068, 0x0331, 0x0000},
    {0x1E97, 0x0074, 0x0308, 0x0000},
    {0x1E98, 0x0077, 0x030A, 0x0000},
    {0x1E99, 0x0079, 0x030A, 0x0000},
    {0x1E9A, 0x0061, 0x02BE, 0x0000},
    {0x1E9E, 0x0073, 0x0073, 0x0000},
    {0x1F50, 0x03C5, 0x0313, 0x0000},
    {0x1F52, 0x03C5, 0x0313, 0x0300, 0x0000},
    {0x1F54, 0x03C5, 0x0313, 0x0301, 0x0000},
    {0x1F56, 0x03C5, 0x0313, 0x0342, 0x0000},
    {0x1F80, 0x1F00, 0x03B9, 0x0000},
    {0x1F81, 0x1F01, 0x03B9, 0x0000},
    {0x1F82, 0x1F02, 0x03B9, 0x0000},
    {0x1F83, 0x1F03, 0x03B9, 0x0000},
    {0x1F84, 0x1F04, 0x03B9, 0x0000},
    {0x1F85, 0x1F05, 0x03B9, 0x0000},
    {0x1F86, 0x1F06, 0x03B9, 0x0000},
    {0x1F87, 0x1F07, 0x03B9, 0x0000},
    {0x1F88, 0x1F00, 0x03B9, 0x0000},
    {0x1F89, 0x1F01, 0x03B9, 0x0000},
    {0x1F8A, 0x1F02, 0x03B9, 0x0000},
    {0x1F8B, 0x1F03, 0x03B9, 0x0000},
    {0x1F8C, 0x1F04, 0x03B9, 0x0000},
    {0x1F8D, 0x1F05, 0x03B9, 0x0000},
    {0x1F8E, 0x1F06, 0x03B9, 0x0000},
    {0x1F8F, 0x1F07, 0x03B9, 0x0000},
    {0x1F90, 0x1F20, 0x03B9, 0x0000},
    {0x1F91, 0x1F21, 0x03B9, 0x0000},
    {0x1F92, 0x1F22, 0x03B9, 0x0000},
    {0x1F93, 0x1F23, 0x03B9, 0x0000},
    {0x1F94, 0x1F24, 0x03B9, 0x0000},
    {0x1F95, 0x1F25, 0x03B9, 0x0000},
    {0x1F96, 0x1F26, 0x03B9, 0x0000},
    {0x1F97, 0x1F27, 0x03B9, 0x0000},
    {0x1F98, 0x1F20, 0x03B9, 0x0000},
    {0x1F99, 0x1F21, 0x03B9, 0x0000},
    {0x1F9A, 0x1F22, 0x03B9, 0x0000},
    {0x1F9B, 0x1F23, 0x03B9, 0x0000},
    {0x1F9C, 0x1F24, 0x03B9, 0x0000},
    {0x1F9D, 0x1F25, 0x03B9, 0x0000},
    {0x1F9E, 0x1F26, 0x03B9, 0x0000},
    {0x1F9F, 0x1F27, 0x03B9, 0x0000},
    {0x1FA0, 0x1F60, 0x03B9, 0x0000},
    {0x1FA1, 0x1F61, 0x03B9, 0x0000},
    {0x1FA2, 0x1F62, 0x03B9, 0x0000},
    {0x1FA3, 0x1F63, 0x03B9, 0x0000},
    {0x1FA4, 0x1F64, 0x03B9, 0x0000},
    {0x1FA5, 0x1F65, 0x03B9, 0x0000},
    {0x1FA6, 0x1F66, 0x03B9, 0x0000},
    {0x1FA7, 0x1F67, 0x03B9, 0x0000},
    {0x1FA8, 0x1F60, 0x03B9, 0x0000},
    {0x1FA9, 0x1F61, 0x03B9, 0x0000},
    {0x1FAA, 0x1F62, 0x03B9, 0x0000},
    {0x1FAB, 0x1F63, 0x03B9, 0x0000},
    {0x1FAC, 0x1F64, 0x03B9, 0x0000},
    {0x1FAD, 0x1F65, 0x03B9, 0x0000},
    {0x1FAE, 0x1F66, 0x03B9, 0x0000},
    {0x1FAF, 0x1F67, 0x03B9, 0x0000},
    {0x1FB2, 0x1F70, 0x03B9, 0x0000},
    {0x1FB3, 0x03B1, 0x03B9, 0x0000},
    {0x1FB4, 0x03AC, 0x03B9, 0x0000},
    {0x1FB6, 0x03B1, 0x0342, 0x0000},
    {0x1FB7, 0x03B1, 0x0342, 0x03B9, 0x0000},
    {0x1FBC, 0x03B1, 0x03B9, 0x0000},
    {0x1FC2, 0x1F74, 0x03B9, 0x0000},
    {0x1FC3, 0x03B7, 0x03B9, 0x0000},
    {0x1FC4, 0x03AE, 0x03B9, 0x0000},
    {0x1FC6, 0x03B7, 0x0342, 0x0000},
    {0x1FC7, 0x03B7, 0x0342, 0x03B9, 0x0000},
    {0x1FCC, 0x03B7, 0x03B9, 0x0000},
    {0x1FD2, 0x03B9, 0x0308, 0x0300, 0x0000},
    {0x1FD3, 0x03B9, 0x0308, 0x0301, 0x0000},
    {0x1FD6, 0x03B9, 0x0342, 0x0000},
    {0x1FD7, 0x03B9, 0x0308, 0x0342, 0x0000},
    {0x1FE2, 0x03C5, 0x0308, 0x0300, 0x0000},
    {0x1FE3, 0x03C5, 0x0308, 0x0301, 0x0000},
    {0x1FE4, 0x03C1, 0x0313, 0x0000},
    {0x1FE6, 0x03C5, 0x0342, 0x0000},
    {0x1FE7, 0x03C5, 0x0308, 0x0342, 0x0000},
    {0x1FF2, 0x1F7C, 0x03B9, 0x0000},
    {0x1FF3, 0x03C9, 0x03B9, 0x0000},
    {0x1FF4, 0x03CE, 0x03B9, 0x0000},
    {0x1FF6, 0x03C9, 0x0342, 0x0000},
    {0x1FF7, 0x03C9, 0x0342, 0x03B9, 0x0000},
    {0x1FFC, 0x03C9, 0x03B9, 0x0000},
    {0xFB00, 0x0066, 0x0066, 0x0000},
    {0xFB01, 0x0066, 0x0069, 0x0000},
    {0xFB02, 0x0066, 0x006C, 0x0000},
    {0xFB03, 0x0066, 0x0066, 0x0069, 0x0000},
    {0xFB04, 0x0066, 0x0066, 0x006C, 0x0000},
    {0xFB05, 0x0073, 0x0074, 0x0000},
    {0xFB06, 0x0073, 0x0074, 0x0000},
    {0xFB13, 0x0574, 0x0576, 0x0000},
    {0xFB14, 0x0574, 0x0565, 0x0000},
    {0xFB15, 0x0574, 0x056B, 0x0000},
    {0xFB16, 0x057E, 0x0576, 0x0000},
    {0xFB17, 0x0574, 0x056D, 0x0000},
    {0x0000}
};


static void string_internal_direct_uppercase_conversion(uint32_t codepoint, StringBuilder stringbuilder);
static size_t string_internal_direct_uppercase_conversion_length(uint32_t codepoint);
static uint32_t string_internal_get_lowercase_codepoint(uint32_t codepoint);
static uint32_t string_internal_ascii_to_lowercase(uint32_t codepoint);
static uint32_t string_internal_ascii_to_uppercase(uint32_t codepoint);


char* string_to_uppercase(const char* str) {
    if (str == NULL) return NULL;
    if (str[0] == '\0') return string_duplicate("");

    Grapheme grapheme;
    size_t str_len = strlen(str);
    StringBuilder stringbuilder = stringbuilder_init(str_len);

    int32_t index = 0;

L_next_grapheme:
    while (string_get_character_codepoint(str, index, str_len, &grapheme)) {
        index += grapheme.size;

        uint32_t ascii_codepoint = string_internal_ascii_to_uppercase(grapheme.code);
        if (ascii_codepoint != 0x00) {
            stringbuilder_add_char_codepoint(stringbuilder, ascii_codepoint);
            continue;
        }

        for (const uint32_t** i = (const uint32_t**)STRING_LOWERCASE_TO_COMPONENTS; **i; i++) {
            const uint32_t* lowercase_components = *i;
            if (*lowercase_components != grapheme.code) continue;

            // decompose components found, unfold each one
            for (; *lowercase_components; lowercase_components++) {
                string_internal_direct_uppercase_conversion(*lowercase_components, stringbuilder);
            }

            goto L_next_grapheme;
        }

        // simple character unfolding
        string_internal_direct_uppercase_conversion(grapheme.code, stringbuilder);
    }

    return stringbuilder_finalize(&stringbuilder);
}

char* string_to_lowercase(const char* str) {
    if (str == NULL) return NULL;
    if (str[0] == '\0') return string_duplicate("");

    Grapheme grapheme;
    size_t str_len = strlen(str);
    StringBuilder stringbuilder = stringbuilder_init(str_len);

    int32_t index = 0;

L_next_grapheme:
    while (string_get_character_codepoint(str, index, str_len, &grapheme)) {
        index += grapheme.size;

        uint32_t ascii_codepoint = string_internal_ascii_to_lowercase(grapheme.code);
        if (ascii_codepoint != 0x00) {
            stringbuilder_add_char_codepoint(stringbuilder, ascii_codepoint);
            continue;
        }

        for (const uint32_t* i = STRING_UPPERCASES; *i; i++) {
            if (*i == grapheme.code) {
                stringbuilder_add_char_codepoint(stringbuilder, STRING_LOWERCASES[i - STRING_UPPERCASES]);
                goto L_next_grapheme;
            }
        }

        // if not found, the codepoint was a number, symbol or emoji
        stringbuilder_add_char_codepoint(stringbuilder, grapheme.code);
    }

    return stringbuilder_finalize(&stringbuilder);
}


size_t string_to_uppercase_length(const char* str) {
    if (str == NULL || str[0] == '\0') return 0;

    Grapheme grapheme;
    size_t str_len = strlen(str);
    size_t length = 0;
    int32_t index = 0;

L_next_grapheme:
    while (string_get_character_codepoint(str, index, str_len, &grapheme)) {
        index += grapheme.size;

        if (grapheme.code <= 0x7F) {
            length++;
            continue;
        }

        for (const uint32_t** i = (const uint32_t**)STRING_LOWERCASE_TO_COMPONENTS; **i; i++) {
            const uint32_t* lowercase_components = *i;
            if (*lowercase_components != grapheme.code) continue;

            // decompose components found, unfold each one
            for (; *lowercase_components; lowercase_components++) {
                length += string_internal_direct_uppercase_conversion_length(*lowercase_components);
            }

            goto L_next_grapheme;
        }

        // simple character unfolding
        length += string_internal_direct_uppercase_conversion_length(grapheme.code);
    }

    return length;
}

size_t string_to_lowercase_length(const char* str) {
    if (str == NULL || str[0] == '\0') return 0;

    Grapheme grapheme;
    size_t str_len = strlen(str);
    size_t length = 0;
    int32_t index = 0;

L_next_grapheme:
    while (string_get_character_codepoint(str, index, str_len, &grapheme)) {
        index += grapheme.size;

        if (grapheme.code <= 0x7F) {
            length++;
            continue;
        }

        for (const uint32_t* i = STRING_UPPERCASES; *i; i++) {
            if (*i == grapheme.code) {
                length += string_length_from_codepoint(STRING_LOWERCASES[i - STRING_UPPERCASES]);
                goto L_next_grapheme;
            }
        }

        // if not found, the codepoint was a number, symbol or emoji
        length += string_length_from_codepoint(grapheme.code);
    }

    return length;
}


bool string_equals_ignore_case(const char* str1, const char* str2) {
    if (str1 == str2) return true; // same pointer
    if (!str1 && !str2) return true;
    if (!str1 || !str2) return false;

    Grapheme grapheme1, grapheme2;
    size_t str1_len = strlen(str1), str2_len = strlen(str2);

    int32_t index1 = 0, index2 = 0;

    while (true) {
        bool has_grapheme1 = string_get_character_codepoint(str1, index1, str1_len, &grapheme1);
        bool has_grapheme2 = string_get_character_codepoint(str2, index2, str2_len, &grapheme2);

        if (!has_grapheme1 && !has_grapheme2)
            return true;
        else if (has_grapheme1 != has_grapheme2)
            break;

        index1 += grapheme1.size;
        index2 += grapheme2.size;

        uint32_t codepoint1 = string_internal_get_lowercase_codepoint(grapheme1.code);
        uint32_t codepoint2 = string_internal_get_lowercase_codepoint(grapheme2.code);

        if (codepoint1 != codepoint2) break;
    }

    return false;
}


static void string_internal_direct_uppercase_conversion(uint32_t codepoint, StringBuilder stringbuilder) {
    uint32_t ascii_codepoint = string_internal_ascii_to_uppercase(codepoint);
    if (ascii_codepoint != 0x00) {
        stringbuilder_add_char_codepoint(stringbuilder, codepoint);
        return;
    }

    for (const uint32_t* i = STRING_LOWERCASES; *i; i++) {
        if (*i == codepoint) {
            codepoint = STRING_UPPERCASES[i - STRING_LOWERCASES];
            break;
        }
    }

    // if not found, the codepoint was a number, symbol or emoji
    stringbuilder_add_char_codepoint(stringbuilder, codepoint);
}

static size_t string_internal_direct_uppercase_conversion_length(uint32_t codepoint) {
    if (codepoint <= 0x7F) {
        // ASCII codepoint
        return 1;
    }

    for (const uint32_t* i = STRING_LOWERCASES; *i; i++) {
        if (*i == codepoint) {
            codepoint = STRING_UPPERCASES[i - STRING_LOWERCASES];
            break;
        }
    }

    return string_length_from_codepoint(codepoint);
}

static uint32_t string_internal_get_lowercase_codepoint(uint32_t codepoint) {
    uint32_t ascii_codepoint = string_internal_ascii_to_lowercase(codepoint);
    if (ascii_codepoint != 0x00) return ascii_codepoint;

    for (const uint32_t* i = STRING_UPPERCASES; *i; i++) {
        if (*i == codepoint) {
            return STRING_LOWERCASES[(size_t)(i - STRING_UPPERCASES)];
        }
    }

    // if not found, the codepoint was a number, symbol or emoji
    return codepoint;
}


static uint32_t string_internal_ascii_to_lowercase(uint32_t codepoint) {
    // if the codepoint is ASCII, do a quick lookup
    if (codepoint >= 0x41 && codepoint <= 0x5A) {
        // A to Z uppercase letters
        return codepoint + 0x20;
    } else if (codepoint <= 0x7F) {
        // symbol, number or control code
        return codepoint;
    } else {
        // unicode codepoint
        return 0x00;
    }
}

static uint32_t string_internal_ascii_to_uppercase(uint32_t codepoint) {
    // if the codepoint is ASCII, do a quick lookup
    if (codepoint >= 0x61 && codepoint <= 0x7A) {
        // a to z lowercase letters
        return codepoint - 0x20;
    } else if (codepoint <= 0x7F) {
        // symbol, number or control code
        return codepoint;
    } else {
        // unicode codepoint
        return 0x00;
    }
}
