...
FontAtlas fontatlas_init(uint8_t* font_data, int32_t font_data_size) {
    ...
    static_assert(sizeof(FT_UInt) >= sizeof(uint32_t), "sizeof(FT_UInt) >= sizeof(uint32_t)");
...
static inline FontKerning* allocate_kernings(FT_Face face, FontCharData* chardata_array, int32_t size) {
    ...
    malloc_assert(kernings_array, FontKerning[]);
...
static inline void build_atlas(FontCharData* array, int32_t array_size, int32_t gaps, int32_t max_dimmen, TextureAtlas* atlas) {
    ...
    malloc_assert(rects, FontCharData[][]);
...
FontCharMap* fontatlas_atlas_build(FontAtlas fontatlas, uint8_t font_height, int8_t gaps, uint32_t* codepoints_to_add) {
    ...
    malloc_assert(chardata, CharData[]);
    ...
    atlas.texture = calloc_chk(1, (size_t)atlas.texture_byte_size);
    malloc_assert(atlas.texture, uint8_t[]);
